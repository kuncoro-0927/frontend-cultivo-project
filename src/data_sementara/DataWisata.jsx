//import {GiChicken,GiSheep,GiKiwiBird,GiCirclingFish,GiPlantSeed,GiButterflyFlower,GiPlantRoots,} from "react-icons/gi";

// DATA WISATA DI BAGIAN AKTIVITAS
export const aktivitasList = [
  {
    title: "Vin's Berry Park, Bandung",
    description:
      "Vin's Berry Park bergerak di bidang agribisnis budidaya dan agrowisata stroberi.",
    detail:
      "SEIN Farm Bandung menawarkan wisata edukasi pertanian, peternakan, dan budi daya ikan dengan pemandangan indah dari kaki Gunung Manglayang. Pengunjung dapat belajar tentang program ketahanan pangan dan memanfaatkan pekarangan melalui konsep Urban Farming, sekaligus menikmati keindahan alam Kota Bandung dari ketinggian.",
    image: "/images/vinsberrypark.svg",
    price: "10.000",
    fasilitas: [
      { label: "Beri Makan Ayam" },
      { label: "Beri Makan Domba" },
      { label: "Beri Makan Burung" },
      { label: "Kolam Pembibitan" },
      { label: "Menanam Sayuran" },
      { label: "Taman Bunga" },
      { label: "Edukasi Hidroponik" },
    ],
    dataimg: [
      {
        img: "/images/bandung/vinsberry/img7.svg",
      },
      {
        img: "/images/bandung/vinsberry/img6.svg",
      },
      {
        img: "/images/bandung/vinsberry/img5.svg",
      },
      {
        img: "/images/bandung/vinsberry/img4.svg",
      },
      {
        img: "/images/bandung/vinsberry/img3.svg",
      },
      {
        img: "/images/bandung/vinsberry/img2.svg",
      },
      {
        img: "/images/bandung/vinsberry/img1.svg",
      },
    ],
    path: "/wisata/vinsberrypark",
  },

  {
    title: "Sein Farm, Bandung",
    description:
      "Sein Farm menawarkan wisata edukasi pertanian, peternakan, dan budi daya ikan",
    image: "/images/seinfarm.svg",
    price: "45.000",
    path: "/wisata/seinfarm",
    fasilitas: "",
    dataimg: [
      {
        img: "/images/bandung/greengarden/img1.svg",
      },
    ],
  },
  {
    title: "Kampoeng Karet, Karanganyar",
    description:
      "Kampoeng Karet merupakan obyek wisata edukasi yang terletak di Desa Puntukrejo",
    image: "/images/kampungkaret.svg",
    price: "55.000",
  },
];

// DATA WISATA DI BAGIAN REKOMENDASI
export const rekomendasiList = [
  {
    title: "Padaawas Hills Pangalengan, Bandung",
    description:
      "Dengan keelokan hamparan kebun teh luas yang menjadi daya tarik tersendiri..",
    image: "/images/paddawas.svg",
    price: "75.000",
  },
  {
    title: "Wisata Telaga Madiredo, Malang",
    description:
      "Di Malang, ada telaga cantik yang tengah ramai menjadi jujugan masyarakat..",
    image: "/images/madiredo.svg",
    price: "75.000",
  },
  {
    title: "Agrowisata Amanah, Karanganyar",
    description:
      "Menikmati sejuknya pemadangan dengan orang-orang terdekat. Selain itu Anda..",
    image: "/images/amanah.svg",
    price: "75.000",
  },
  {
    title: "Agrowisata Wonosari, Malang",
    description: "Kawasan destinasi wisata alam, edukasi hingga penginapan.",
    image: "/images/wonosari.svg",
    price: "15.000",
  },
];

export const daerahList = [
  {
    title: "Bandung",
    description:
      "Dengan keelokan hamparan kebun teh luas yang menjadi daya tarik tersendiri..",
    image: "/images/bandung.svg",
    price: "75.000",
  },
  {
    title: "Malang",
    description:
      "Dengan keelokan hamparan kebun teh luas yang menjadi daya tarik tersendiri..",
    image: "/images/malang.svg",
    price: "75.000",
  },
  {
    title: "Solo",
    description:
      "Dengan keelokan hamparan kebun teh luas yang menjadi daya tarik tersendiri..",
    image: "/images/solo.svg",
    price: "75.000",
  },
  {
    title: "Yogyakarta",
    description:
      "Dengan keelokan hamparan kebun teh luas yang menjadi daya tarik tersendiri..",
    image: "/images/yogyakarta.svg",
    price: "75.000",
  },
  {
    title: "Surabaya",
    description:
      "Dengan keelokan hamparan kebun teh luas yang menjadi daya tarik tersendiri..",
    image: "/images/surabaya.svg",
    price: "75.000",
  },
  {
    title: "Bogor",
    description:
      "Dengan keelokan hamparan kebun teh luas yang menjadi daya tarik tersendiri..",
    image: "/images/bogor.svg",
    price: "75.000",
  },
  {
    title: "Jakarta",
    description:
      "Dengan keelokan hamparan kebun teh luas yang menjadi daya tarik tersendiri..",
    image: "/images/jakarta.svg",
    price: "75.000",
  },
];
// DATA WISATA SOLO
export const soloList = [
  {
    title: "Selo Tumpang, Boyolali",
    description:
      "Wisatawan dapat mengikuti kegiatan petik buah dan belajar tentang pertanian di daerah pegunungan. ",
    image: "/public/images/solo/selotumpang.svg",
    price: "5.000",
    path: "/wisata/selotumpangboyolali",
  },
  {
    title: "Kampoeng Karet, Karanganyar",
    description:
      "Tempat ini menawarkan pengalaman belajar budidaya karet, dari penyadapan hingga pengolahan getah.",
    image: "/public/images/solo/kampungkaret.svg",
    price: "10.000",
    path: "#",
  },
  {
    title: "Agrowisata Pasung, Klaten",
    description:
      "Desa Pasung memiliki lebih dari 1.000 tanaman buah yang berada di pinggir jalan utama",
    image: "/public/images/solo/desapasung.svg",
    price: "Free",
    path: "#",
  },
  {
    title: "Agrowisata Jambu Merah",
    description:
      "Wisatawan dapat memetik jambu sepuasnya, belajar merawatnya, dan mendapatkan bibit jambu merah.",
    image: "/public/images/solo/puja.svg",
    price: "5.000",
    path: "#",
  },
  {
    title: "Agrowisata Amanah",
    description:
      "Tempat ini tidak hanya menawarkan sektor pertanian dan peternakan, namun juga unsur wisata edukasi didalamnya.",
    image: "/public/images/solo/amanah.svg",
    price: "115.000",
    path: "#",
  },
];

// DATA WISATA YOGYAKARTA

export const yogyakartaList = [
  {
    title: "Agrowisata Banyu Sumilir",
    description:
      "agrowisata ini menawarkan wisata edukasi tentang pertanian organik dan peternakan.",
    image: "/public/images/yogyakarta/banyumili.svg",
    price: "45.000",
  },
  {
    title: "Agrowisata Bhumi Merapi",
    description:
      "Tempat ini menawarkan kegiatan edukatif seperti peternakan, pertanian, dan kebun sayur. ",
    image: "/public/images/yogyakarta/bumimerapi.svg",
    price: "30.000",
  },
  {
    title: "Kebun Buah Mangunan",
    description:
      " Berlokasi di Bantul, terkenal dengan pemandangan alam dan kebun buah",
    image: "/public/images/yogyakarta/mangunan.svg",
    price: "5.000",
  },
  {
    title: "Kebun Buah Mangunan",
    description:
      "Wisatawan bisa memetik buah langsung dari kebun dan menikmati produk olahannya.",
    image: "/public/images/yogyakarta/turi.svg",
    price: "15.000",
  },
  {
    title: "Agrowisata Kalibiru",
    description:
      "Kalibiru menawarkan kegiatan edukasi tentang konservasi alam dan ekowisata",
    image: "/public/images/yogyakarta/kalibiru.svg",
    price: "10.000",
  },
];

// DATA WISATA MALANG
export const malangList = [
  {
    title: "Telaga Madiredo",
    description:
      "Telaga Madiredo merupakan sumber mata air asri yang dikelola langsung oleh pemuda setempat",
    image: "/public/images/malang/telagamadiredo.svg",
    detail: "tes",
    price: "45.000",
    dataimg: [
      {
        img: "/public/images/bandung/vinsberry/img7.svg",
      },
    ],
    path: "/wisata/madiredo",
  },
  {
    title: "Agrowisata Wonosari",
    description:
      "Wisata Agro Wonosari merupakan kawasan destinasi wisata alam, edukasi hingga penginapan.",
    image: "/public/images/malang/wonosari.svg",
    price: "15.000",
    dataimg: [
      {
        img: "/public/images/bandung/vinsberry/img7.svg",
      },
    ],
    detail: "",
    path: "/wisata/agrowisatawonosari",
  },
  {
    title: "Green Garden",
    description:
      "Nikmati pengalaman seru memetik apel jenis manalagi dan apel anna langsung dari pohonnya.",
    image: "/public/images/malang/greengarden.svg",
    price: "25.000",
    dataimg: [
      {
        img: "/public/images/bandung/vinsberry/img7.svg",
      },
    ],
    detail: "",
  },
  {
    title: "Rimba Raya Bee Farm",
    description:
      "Rimba Raya Bee Farm di Lawang, Malang, menawarkan petik madu dan produk lebah terlengkap.",
    image: "/public/images/malang/petikmadu.svg",
    price: "25.000",
    dataimg: [
      {
        img: "/public/images/bandung/vinsberry/img7.svg",
      },
    ],
    detail: "",
  },
  {
    title: "Wisata Petik Jeruk",
    description:
      "Pengunjung dapat memetik jeruk dan menyaksikan proses penanaman serta perawatannya dengan benar.",
    image: "/public/images/malang/petikjeruk.svg",
    price: "20.000",
    dataimg: [
      {
        img: "/public/images/bandung/vinsberry/img7.svg",
      },
    ],
    detail: "",
  },
  // ... data lainnya
];
